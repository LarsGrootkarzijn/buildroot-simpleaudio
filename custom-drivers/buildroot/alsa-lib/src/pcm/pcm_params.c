			      ...
			      unsigned int *val, int *dir)
{
	...
	unsigned int tempdir = *dir;
	snd_interval_t *i;
	static int lastclock = 1;
	
	snd_pcm_hw_t *hw = pcm->private_data;

	if(var == SND_PCM_HW_PARAM_RATE)
	{
		snd_pcm_hw_params_set_rate(pcm, params,*val, 0);
		return 0;
	}
	/* FIXME */
...
{
	char system_buffer[SA_SYSTEM_BUFFER];
	...
	float clkselect = (float)SA_24_576_MHZ_CLOCK / (float)rate;
    	int clk = SA_CLOCK_SELECT_11_2896_MHZ;

    	if(floorf(clkselect) == (float)clkselect)
    	{
    		clk = SA_CLOCK_SELECT_24_576_MHZ;
    	}
	return clk;
}
...
{
    	long mcasp = syscall(SYSCALL_CLOCK_CHANGE, clock);
}

void set_kernel_clock(unsigned int rate)
{
	static int last_rate;
	if(rate != last_rate)
	{
		set_kernel_snd_card_clock(get_clock_to_set(rate));
	}
}
...
void set_kernel_clock_gpio(unsigned int rate)
{
	...
	if(rate != last_rate)
	{
		set_clock_gpio(get_clock_to_set(rate));
	}
}
...
{
	FILE *sysfs_handle;
	if ((sysfs_handle = fopen(path, SA_GPIO_WRITE)) != NULL)
    {
		int written = fprintf(sysfs_handle, "%d", message);
		fclose(sysfs_handle);
		return written;
	}
	else
...
{	
	...
	if(clk != old_clk)
	{
		...
		if(access(SA_GPIO_492_VALUE, F_OK) == 0) 
		{
			write_file(SA_GPIO_492_VALUE, clk);
		}
		else
		{	
			write_file(SA_GPIO_EXPORT, SA_GPIO_492);
 
			do
			{
				err = write_file(SA_GPIO_492_VALUE, clk);
			}
			while(err < 0);
		}

		if(access(SA_GPIO_491_VALUE, F_OK) == 0) 
		{
			write_file(SA_GPIO_491_VALUE, clk);
		}	
		else
		{
			write_file(SA_GPIO_EXPORT, SA_GPIO_491);

			do
			{
				err = write_file(SA_GPIO_491_VALUE, clk);
			}
			while(err < 0);
		}
	}
