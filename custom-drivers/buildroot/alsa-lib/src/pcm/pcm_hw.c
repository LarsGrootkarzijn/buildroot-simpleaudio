...
static inline int hw_refine_call(snd_pcm_hw_t *pcm_hw, snd_pcm_hw_params_t *params)
{
	...
	err = snd_pcm_hw_params_get_rate_max(params, &val, 0);
	//printf("rate max %d \n", val);
	//fflush(stdout);
	if (err < 0)
		printf("error reading rate code = %d", err);

	if(val > 0 && val <= SA_MAX_RATE && pcm_hw->card == SA_CARD_NUMBER && pcm_hw->device == SA_DEVICE_NUMBER && pcm_hw->subdevice == SA_SUBDEVICE_NUMBER)
	{
		set_kernel_clock(val);
		set_kernel_clock_gpio(val);
	}
...
static int snd_pcm_hw_hw_refine(snd_pcm_t *pcm, snd_pcm_hw_params_t *params)
{
	...
	if(hw->card == SA_CARD_NUMBER && hw->device == SA_DEVICE_NUMBER && hw->subdevice == SA_SUBDEVICE_NUMBER)
		set_kernel_clock(SA_MAX_RATE);
